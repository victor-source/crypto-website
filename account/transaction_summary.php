                        ...
                        <div class="card-body center">
                            <div class="table-responsive">
                                <table class="table table-bordered center table-hover" id="dt-addrows">
                                    ...
                                    <tbody>
                                        <?php
                                        $trans = $conn->query("select * from transactions where email='$user' order by id DESC");
                                        foreach($trans as $tran){
                                        ?>
                                        <tr>
                                            <td><?php if($tran['type']=="Withdrawal"){?><button type="button" class="btn btn-success">Withdrawal</button><?php }else if($tran['type']=="Deposit"){?><button type="button" class="btn btn-success">Deposit</button><?php }?></td>
                                            <td><?php echo $tran['amount']?></td>
                                            <td><?php echo $tran['method']?></td>
                                            <td><?php echo $tran['date']?></td>
                                            <td><?php if($tran['status']==0){?><button type="button" class="btn btn-danger">Processing</button><?php }else if($tran['status']==1){?><button type="button" class="btn btn-success">Completed</button><?php }?></td>
